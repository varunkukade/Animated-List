import {TSongPositions, TSongsHeight} from './types';
...
export const getInitialPositions = (): TSongPositions => {
  ...
  for (let i = 0; i < SONGS.length; i++) {
    songPositions[i] = {
      updatedIndex: i,
      updatedTop: 0,
      updatedTopWhileDragging: 0,
    };
  }
  return songPositions;
};

export const getInitialHeights = (): TSongsHeight => {
  let initialHeights: TSongsHeight = {};
  for (let i = 0; i < SONGS.length; i++) {
    initialHeights[i] = 0;
  }
...
export const getSum = (object: Object, sumUpto?: number) => {
  let sum = 0;
  let arr = Object.values(object);
  for (let i = 0; i < (sumUpto ? sumUpto : arr.length); i++) {
    sum = sum + arr[i];
  }
  return sum;
};
